...
function buildIndex (from, to, options, notify, addAssets) {
  ...
  if(!options.skipData) {
    var dataDir = path.join(from, options.data);
    var data = datapop.buildData( dataDir, "", notify );
    datapop.validate(slidesDir, data, notify);
  }
  ...
  //-- Replace mustache {{x.y}} tokens
  if(!options.skipData) {
    html = datapop.render(html, data);
  }
  write(path.join(to, "index.html"), html, notify);

  //-- Special handling of custom.css, Replace mustache {{x.y}} tokens
  if(!options.skipData) {
      console.log("Special handling of custom.css", data.color_theme);
      var css = fs.readFileSync(path.join(from, "css", "custom.css"), "utf8");
      css = datapop.render(css, data);
      fs.writeFileSync(path.join(to, "css", "custom.css"), css, "utf8");
  }
